...
if __name__ == '__main__':
    ...
    train_tfidf = transformer.fit_transform(train_corpus)
    test_tfidf = transformer.transform(test_corpus)
    ...
    clf = svm.SVC()
    clf.fit(train_weight, train_news_ca)

    i = 0
    while 1:
        print(test_corpus[i])
        print(test_news_ca[i])
        # predict the target of testing samples
        print(test_weight[i])
        result = clf.predict([test_weight[i]])
        print(result)  # ta
        i += 1
        if(i == 2):
            break
